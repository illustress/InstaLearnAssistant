...
import { Send, Loader2, Bot, User, Trash2 } from 'https://esm.sh/lucide-react@0.263.1?deps=react@18.2.0';
...
export const ChatView = () => {
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      setMessages(prev => [...prev, { role: 'model', text: 'Sorry, er ging iets mis. Controleer je API key.', isError: true }]);
    } finally {
      setLoading(false);
    }
  };
      ...
      <div className="input-area">
         <div className="input-wrapper">
            ...
            <button className="send-btn" onClick=${handleSend} disabled=${loading || !input.trim()}>
                ${loading ? html`<${Loader2} className="animate-spin" size=${16} />` : html`<${Send} size=${16} />`}
            </button>
         </div>
      </div>
    </div>
