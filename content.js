...
async function loadData() {
  return new Promise((resolve) => {
    try {
      ...
      chrome.storage.sync.get(['direction', 'credits', 'streak'], (syncData) => {
        if (chrome.runtime?.lastError) { markExtensionContextInvalid(chrome.runtime.lastError); resolve(); return; }
        if (syncData.direction) settings.direction = syncData.direction;
        if (typeof syncData.credits === 'number') credits = syncData.credits;
        if (typeof syncData.streak === 'number') streak = syncData.streak;

        chrome.storage.local.get(['wordProgress', 'customWords'], (localData) => {
          if (chrome.runtime?.lastError) { markExtensionContextInvalid(chrome.runtime.lastError); resolve(); return; }
          if (localData.wordProgress) wordProgress = localData.wordProgress;
          words = localData.customWords?.length > 0 ? localData.customWords : DEFAULT_WORDS;
          startSession();
          resolve();
        });
      });
...
function saveProgress() {
  ...
  } catch (e) { markExtensionContextInvalid(e); }
}
...
window.addEventListener('error', (event) => {
  if (handleInvalidatedContextError(event.error || event.message)) event.preventDefault();
});
...
function markExtensionContextInvalid(error) {
  ...
  console.warn('[InstaLearning] Extension context invalid. Reload the tab after extension reload.', error);
}
...
function showScrambleGame() {
  ...
  let scrambled = answer.split('').sort(() => Math.random() - 0.5).join('');
  // Ensure scrambled is different from answer
  let attempts = 0;
  while (scrambled === answer && attempts < 10) {
    scrambled = answer.split('').sort(() => Math.random() - 0.5).join('');
    attempts++;
  }
...
function showHangmanGame() {
  ...
  function renderHangman() {
    const display = Array.from(answer).map(ch => {
    ...
    content.querySelectorAll('.il-key:not(.used)').forEach(btn => {
      btn.addEventListener('click', () => {
        ...
        const won = answerChars.every(ch => !isLetter(ch) || guessed.has(ch));
      ...
      btn.addEventListener('click', () => {
        ...
        if (selected === null) {
          ...
          if (prevCard.pairId === card.pairId && prevCard.type !== card.type) {
            ...
            card.selected = true;
            renderCards();
            const wrongEls = content.querySelectorAll('.il-match-card.selected');
...
function handleSwipe() {
  console.log(`[InstaLearning] Swipe on ${location.pathname}, credits: ${credits}`);
